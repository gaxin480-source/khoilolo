document.addEventListener("DOMContentLoaded", async () => {
          ...
          <div class="player-column">
            <div class="player-shell">
              ...
              <div class="player-info">
                <h2 class="player-anime-title">${window.StreamUI.escapeHtml(anime.title)}</h2>
                <div class="player-episode">Tập ${episodeNumber}</div>
                <p class="player-description">${window.StreamUI.escapeHtml(anime.description)}</p>
              </div>
            </div>
          </div>
          <aside class="sidebar">
            <div class="sidebar-head"><h3>Episodes</h3></div>
            <div class="episode-list">
              ${allEpisodes.map(episode => {
                const active = Number(episode.seasonNumber) === Number(seasonNumber) &&
                              Number(episode.number) === Number(episodeNumber);
                return `
                  <button class="episode-button ${active ? "is-active" : ""}"
                    ...
                    <h4 class="episode-title">${window.StreamUI.escapeHtml(anime.title)}</h4>
                  </button>
                `;
              }).join("")}
            </div>
  ...
  function bindVideoEvents(video) {
    if (boundVideo && boundVideo !== video) {
      if (boundVideo.__onTimeUpdate) boundVideo.removeEventListener("timeupdate", boundVideo.__onTimeUpdate);
      if (boundVideo.__onEnded) boundVideo.removeEventListener("ended", boundVideo.__onEnded);
    }
    ...
    if (video.__onTimeUpdate) video.removeEventListener("timeupdate", video.__onTimeUpdate);
    if (video.__onEnded) video.removeEventListener("ended", video.__onEnded);
  ...
  async function attachPlayer() {
    ...
    if (/\.m3u8($|\?)/i.test(streamUrl)) {
      if (window.Hls && window.Hls.isSupported()) {
        ...
        hls.on(window.Hls.Events.MEDIA_ATTACHED, () => {
          console.log("✅ HLS attached → tự động play");
    ...
    setupThumbnailPreview(video, { ...currentEpisode, thumbnails: thumbnailUrl });
  ...
  async function setupThumbnailPreview(video, episode) {
    ...
    const leaveHandler = () => { preview.style.display = "none"; };
